...
impl <const CAPACITY: usize> LowestIntegerCache<CAPACITY> {
    ...
    pub fn get_lowest_integer(& self) -> Option<usize> {
        for idx in (self.watermark/BIT_PER_DWORD)..(CAPACITY) {
            let v = self.bitfield[idx];
            if !v == 0 {
                continue;
            }
            if v == 0 {
                return Some(idx * BIT_PER_DWORD);
            }

            let lowest_zero = (!v) & (!v).wrapping_neg();
            return Some(lowest_zero.trailing_zeros() as usize + idx * BIT_PER_DWORD);
        }
        return None;
    }
    ...
    pub fn allocate(&mut self) -> Option<usize> {
        ...
        if let Some(v) = ret {
            self.watermark = v + 1;
            let (q, r) = v.div_rem_euclid(&BIT_PER_DWORD);
            self.bitfield[q] |= (1 << r);
        }
        ret
    }
    ...
    pub fn free(&mut self, x: usize) {
        assert!(x < CAPACITY * BIT_PER_DWORD);

        self.watermark = std::cmp::min(x, self.watermark);
        let (q, r) = x.div_rem_euclid(&BIT_PER_DWORD);
        self.bitfield[q] &= !(1 << r);
    }
}
...
mod test {
    ...
    fn test_lowest_integer_cache_uniform() {
        ...
        for i in 0..(TEST_CAPACITY*BIT_PER_DWORD) {
            assert_eq!(t.get_lowest_integer().unwrap(), i);
            t.allocate();
        }
        assert_eq!(t.get_lowest_integer(), None);

        for i in (0..(TEST_CAPACITY*BIT_PER_DWORD)).rev() {
            t.free(i);
    ...
    fn test_lowest_integer_cache_sparse() {
        let mut t = LowestIntegerCache::<TEST_CAPACITY>::new();
        for _ in 0..(TEST_CAPACITY*BIT_PER_DWORD) {
            t.allocate();
        }
