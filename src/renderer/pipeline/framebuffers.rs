...
pub struct Framebuffers {
    ...
    hdr_fb      : Option<(wgpu::TextureView, u32, u32)>,
...
impl Framebuffers {
    ...
    pub fn configure_with_surface(&mut self, di: &DeviceInterface) {
        ...
        let need_rebuild = if let Some(t) = self.hdr_fb.as_ref() {
            !(t.1 == width && t.2 == height)
        } else {
            true
        };
        ...
        if need_rebuild {
            ...
            let texture_view = di.get_device().create_texture(&wgpu::TextureDescriptor{ 
            ...
            self.hdr_fb = Some((texture_view, width, height));
        }
    ...
    pub fn tonemap_with(&self, di: &DeviceInterface, ce: &mut wgpu::CommandEncoder, final_texture_view: &wgpu::TextureView) {
        ...
        let bind_group = di.get_device().create_bind_group(&wgpu::BindGroupDescriptor { 
            ...
            entries: &[
                wgpu::BindGroupEntry{
                    binding: 0,
                    resource: wgpu::BindingResource::TextureView(&self.hdr_fb.as_ref().expect("call configure_with_surface before this method.").0)
                },
                wgpu::BindGroupEntry{
                    binding: 1,
